...
const workspaceGlobs = [
  ...
  { dir: 'native', depth: 1 },
  { dir: 'packages/native/landlock-run/packages', depth: 1 },
  { dir: 'apps', depth: 1 },
] as const
...
const landlockWorkspaceManifest = readJson(join(root, 'packages/native/landlock-run/package.json'))
...
function checkWorkspace({ dir, manifest }: WorkspaceManifest): string[] {
  ...
  const label = manifest.name ?? dir
  const isLandlockPackageDir = dir.startsWith('packages/native/landlock-run/packages/')
