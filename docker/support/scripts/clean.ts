...
export class RepositoryCleaner {
  ...
  private async plan(): Promise<string[]> {
    ...
    await this.addIfPresent(
      targets,
      join(this.root, 'packages/native/landlock-run/tsconfig.tsbuildinfo'),
      canonicalRoot,
    )
  ...
  private buildOutputDirectories(): string[] {
    ...
    const nativeEntryOutput = join(this.root, 'packages/native/landlock-run/packages/entry/lib')
