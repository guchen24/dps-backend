...
签入仓库的会话 fixture 保留完整会话 header 和事件 payload，但省略普通行的 `seq`/`time` 与打包行的 `seq0`/`time0` envelope。回放只在内存中合成这些 envelope；运行时持久化不变。fixture 同时使用规范打包行；[临时仓库迁移器](../../../docker/support/scripts/migrate-packed-session-fixtures.ts)（`pnpm run migrate:packed-session-fixtures`）会改写较旧的 fixture 布局，由其[移除提案](../../../.agents/notes/proposed/process/2026-07-26-remove-packed-session-fixture-migrator.zh.md)负责删除该迁移器。
