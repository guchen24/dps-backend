...
Committed session fixtures retain their complete session header and event payloads but omit ordinary `seq`/`time` and packed-row `seq0`/`time0` envelopes. Replay synthesizes those envelopes in memory; runtime persistence remains unchanged. Fixtures also use canonical packed rows; the [temporary repository migrator](../../../docker/support/scripts/migrate-packed-session-fixtures.ts) (`pnpm run migrate:packed-session-fixtures`) rewrites older fixture layouts, and its [removal proposal](../../../.agents/notes/proposed/process/2026-07-26-remove-packed-session-fixture-migrator.md) owns its deletion.
